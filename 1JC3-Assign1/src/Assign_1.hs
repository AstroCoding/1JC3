...
{-
    ...
    - cubicRealSolutions 1 1 1 1 = [-1] - Works
 -}

 {-
    Finally add an if statement that checks for NaN and returns an empty list.

    An imaginary number can still be multiplied into a real number, the the final answers aren't actually impacted.
 -}
cubicRealSolutions :: Double -> Double -> Double -> Double -> [Double]
cubicRealSolutions 0 b c d = error "You cannot run the function on a Quadratic Formula"
cubicRealSolutions a b c d = do
    let q = cubicQ a b c
        r = cubicR a b c d
    if (cubicDisc q r < 0 || a == 0) then []
    else
        let
            q = cubicQ a b c
            r = cubicR a b c d
        ...
        in if (cubicDisc q r > 0) then [x1]
            else [x1, x2, x3]
    {-
        Positive discriminants are still being filtered into the == 0 if statement. I had to implement a hard fix, and it is not the prettiest, believe me.
    -}
